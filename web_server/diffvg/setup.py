    ...
    def build_extension(self, ext):
        if isinstance(ext, CMakeExtension):
            ...
            if platform.system() == "Windows":
                python_lib = os.path.join(sys.exec_prefix, 'libs', f'python{sys.version_info.major}{sys.version_info.minor}.lib')
                # Replace backslashes with forward slashes for CMake path compatibility
                python_lib = python_lib.replace('\\', '/')
            else:
                python_lib = get_config_var('LIBDIR') or ''

            python_executable = sys.executable.replace('\\', '/')
            python_root = sys.exec_prefix.replace('\\', '/')
